...
def analyze_project_structure() -> dict:
    ...
    for item in PROJECT_ROOT.iterdir():
        if item.is_dir() and item.name.startswith(("0", "1")):
            module_info = {"name": item.name, "submodules": []}
    ...
    for _nb_file in PROJECT_ROOT.rglob("*.ipynb"):
        stats["total_files"] += 1
...
def run_module_tests(module_name: str) -> dict:
    ...
    try:
        ...
        result = subprocess.run(
            ["pytest", str(module_path), "-v"], capture_output=True, text=True, timeout=300
        )
        ...
        return {"success": False, "error": "Test execution timeout (5 minutes)"}
...
@mcp.tool()
def list_notebooks(module_name: str | None = None) -> list:
    """
    ...
    Returns:
        list: notebook 文件列表
    """
    search_path = PROJECT_ROOT / module_name if module_name else PROJECT_ROOT
...
def get_project_stats() -> dict:
    ...
    # 统计 notebooks
    for _nb_file in PROJECT_ROOT.rglob("*.ipynb"):
        if ".ipynb_checkpoints" not in str(_nb_file):
    ...
    for file_path in PROJECT_ROOT.rglob(pattern):
        ...
        try:
            ...
            if query.lower() in content.lower():
                ...
                lines = content.split("\n")
                matching_lines = [
                    (i + 1, line) for i, line in enumerate(lines) if query.lower() in line.lower()
                ]
...
def get_module_dependencies(module_name: str) -> dict:
    ...
    return {"module": module_name, "dependencies": sorted(imports), "count": len(imports)}
